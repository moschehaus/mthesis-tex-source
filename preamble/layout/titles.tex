...
%% FIX: zref-clever recognises the reference type from \refstepcounter, but for
%%   unnumbered sectioning levels (secnumdepth < level) \refstepcounter is skipped.
%%   This causes zref to inherit the type from the last stepped counter (e.g. sidenote).
%%   The \titleformat before-code runs inside titlesec's \begingroup, so we use \gdef
%%   to globally set the zref internal `zc@type` property to the correct sectioning type.
%%   For numbered levels, \refstepcounter will overwrite this with the same value anyway.
