...
\newcommand*{\Rplusnought}{\mathbb{R}^{+}_{0}}
...
%%% Function spaces
\newcommand*{\Loneloc}[1]{L^{1}_{\mathrm{loc}}(#1)}
\NewDocumentCommand{\Czero}{g}{\IfValueTF{#1}{C(#1)}{C}}
\NewDocumentCommand{\Cone}{g}{\IfValueTF{#1}{C^{1}(#1)}{C^{1}}}
\NewDocumentCommand{\Ctwo}{g}{\IfValueTF{#1}{C^{2}(#1)}{C^{2}}}
\NewDocumentCommand{\Cinf}{g}{\IfValueTF{#1}{C^{\infty}(#1)}{C^{\infty}}}
\NewDocumentCommand{\Ccinf}{g}{\IfValueTF{#1}{C^{\infty}_0(#1)}{C^{\infty}_0}}

%%% SPH operators
\newcommand*{\sphc}[1]{[#1]_{c}}
\newcommand*{\sphd}[1]{[#1]_{d}}
...
\makeatletter
\DeclarePairedDelimiterX{\pbr@inner}[2]{\lbrace}{\rbrace}{#1,#2}
\NewDocumentCommand{\pbr}{O{} m m}{\pbr@inner*{#2}{#3}^{\mkern-2mu\text{(#1)}}}
\makeatother
...
%% Ordinary derivative (replaces physics package \dv)
%% Usage:
%% \dv{t}       -> d/dt  (operator form)
%% \dv{f}{x}    -> df/dx
%% \dv[2]{f}{x} -> d^2f / dx^2  (higher order)
\NewDocumentCommand{\dv}{O{} m g}{%
    \IfNoValueTF{#3}{%
        % One mandatory arg: operator form d/dt
        \IfBlankTF{#1}{%
...
%% Usage:
%% \pdv{f}{x}    -> ∂f/∂x
%% \pdv{x}       -> ∂/∂x  (operator form)
%% \pdv[2]{f}{x} -> ∂²f/∂x²  (higher order)
...
%% Usage:
%% \fdv{F}{u}    -> δF/δu
%% \fdv{}{u}     -> δ/δu  (operator form)
%% \fdv[2]{F}{u} -> δ²F/δu²  (higher order)
\NewDocumentCommand{\fdv}{O{} m m}{%
    \IfBlankTF{#1}{%
...
%% Scaled-up integral sign (matches tall \left/\right delimiters)
\NewDocumentCommand{\bigint}{e{_}}{\mathop{\scalemath{1.2}[1.6]{\int}}\limits\IfValueT{#1}{_{#1}}\!\!}
...
% Clear the default division symbol to redefine \div
\let\div\relax
\DeclareMathOperator{\div}{div}
\NewDocumentCommand{\divergence}{}{\nabla \cdot}

% Curl operators
\DeclareMathOperator{\curl}{curl}
\NewDocumentCommand{\curlop}{}{\nabla \times}

\usepackage{accents}

%% Large, properly spaced time derivative dots
%% Relies on your custom \scalemath and \raisemath macros
\NewDocumentCommand{\tdot}{m}{%
    \accentset{\raisemath{-0.15ex}{\scalemath{1.5}{\bm{\cdot}}}}{#1}%
}

\NewDocumentCommand{\tddot}{m}{%
    \accentset{\raisemath{-0.15ex}{\scalemath{1.5}{\bm{\cdot}\mspace{-3mu}\bm{\cdot}}}}{#1}%
}
